...
boru-driven command-line client **and** interactive REPL for the KorpoPro
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/korpo-pro-cli)
make build
...
./korpo-pro-cli list health

# 5. Override the API base URL for a single call
KORPO_PRO_BASE=https://api.example.com ./korpo-pro-cli list health

# 6. No arguments -> interactive REPL
./korpo-pro-cli
korpo-pro> list health
korpo-pro> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/korpo-pro-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/korpo-pro-cli list health
   ```
...
Run with no arguments to open a REPL (prompt `korpo-pro>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/korpo-pro-cli`. |
...
(`./korpo-pro-cli <expr>`) and interactively (the REPL), and expressions compose the
