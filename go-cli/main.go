// Command KorpoPro-cli is a CLI + REPL for the KorpoPro SDK,
...
const prompt = "korpo-pro"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "health"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KORPO_PRO_APIKEY (api key), KORPO_PRO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KORPO_PRO_APIKEY carries the API key and
	// KORPO_PRO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KORPO_PRO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KORPO_PRO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKorpoProSDK(opts)
