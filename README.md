...
// healths is an array of Health entities, populated with mock data
// — call healths[0].data() for the record itself
console.log(healths)
```
...
// List all healths (returns HealthEntity[] — .data() for the record)
const healths = await client.Health().list()
...
$healths = $client->Health()->list();
print_r(array_map(fn($item) => $item->data_get(), $healths));
```
