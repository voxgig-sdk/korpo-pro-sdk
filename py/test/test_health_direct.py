...
class TestHealthDirect:
    ...
    def test_should_direct_list_health(self):
        setup = _health_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-health", "live" if setup["live"] else "unit")
        if _skip:
...
def _health_direct_setup(mockres):
    ...
    if live:
        ...
        client = KorpoProSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
