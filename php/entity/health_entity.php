...
class HealthEntity
{
    ...
    {
        if ($args) {
            $this->_data = KorpoProHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = KorpoProHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    /**
     * List Health items matching the given filter.
     *
     * @param HealthListMatch|array|null $reqmatch Match filter (any subset
     *   of Health fields) as an assoc-array; HealthListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Health[]|array A list of Health items as assoc-arrays at
     *   the SDK boundary; throws KorpoProError on failure (item-5 convention).
     */
