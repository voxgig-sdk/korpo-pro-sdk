...
class KorpoProNetsimFeature extends KorpoProBaseFeature
{
    ...
    private function _simulate(KorpoProContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(KorpoProContext $ctx, array $applied): void
    {
        $client = $this->client;
