...
local KorpoProSDK = {}
KorpoProSDK.__index = KorpoProSDK
...
KorpoProSDK._make_feature = _make_feature


function KorpoProSDK.new(options)
  local self = setmetatable({}, KorpoProSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function KorpoProSDK:options_map()
...
function KorpoProSDK:get_utility()
  return Utility.copy(self._utility)
end


function KorpoProSDK:get_root_ctx()
  return self._rootctx
end


function KorpoProSDK:prepare(fetchargs)
...
function KorpoProSDK:direct(fetchargs)
...
-- by passing it.
function KorpoProSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function KorpoProSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "KorpoProSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function KorpoProSDK:Health(data)
...
function KorpoProSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = KorpoProSDK.new(sdkopts)
  sdk.mode = "test"
