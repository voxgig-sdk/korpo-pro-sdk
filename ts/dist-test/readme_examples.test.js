...
const TEST_SEED = { "entity": { "health": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+KorpoProSDK\b/.test(code) ||
        /\bKorpoProSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+KorpoProSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/KorpoProSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
