...
(0, node_test_1.describe)('HealthDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-health', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-health', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.KORPO_PRO_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.KorpoProSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['KORPO_PRO_TEST_HEALTH_ENTID'];
    ...
    const client = new __1.KorpoProSDK({
        base: 'http://localhost:8080',
