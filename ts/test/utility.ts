...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.KORPOPRO_TEST_LIVE ||
    'TRUE' === process.env.KORPOPRO_TEST_OVERRIDE
  ) {
  ...
  m.KORPOPRO_TEST_EXPLAIN = process.env.KORPOPRO_TEST_EXPLAIN || m.KORPOPRO_TEST_EXPLAIN
