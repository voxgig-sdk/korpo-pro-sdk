...
class HealthEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = KorpoProSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KORPOPRO_TEST_HEALTH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def health_basic_setup(extra)
  ...
  entid_env_raw = ENV["KORPOPRO_TEST_HEALTH_ENTID"]
  ...
  if env["KORPOPRO_TEST_LIVE"] == "TRUE"
    ...
    client = KorpoProSDK.new(Helpers.to_map(merged_opts))
  end
