...
class HealthEntity
  ...
  def data_set(args)
    if args
      @_data = KorpoProHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = KorpoProHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [HealthListMatch, Hash, nil] match filter (any subset of
  #   Health fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Health>, Array] the matching Health items; raises KorpoProError on failure
