...
[MCP](https://modelcontextprotocol.io) server exposing the KorpoPro SDK as
two agent tools — `korpo-pro_list` and `korpo-pro_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/korpo-pro-mcp)
make build

# 2. Provide credentials via the environment
export KORPO_PRO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user korpo-pro \
  -- /absolute/path/to/korpo-pro-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./korpo-pro-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// korpo-pro_list: first page of records
{ "entity": "health" }
{ "entity": "health", "query": { } }

// korpo-pro_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/korpo-pro-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user korpo-pro \
     -- "$PWD"/dist/*/korpo-pro-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `korpo-pro_list` and `korpo-pro_load` tools now appear
   in new sessions. Ask the agent to *"list health using korpo-pro"*
   and it calls `korpo-pro_list` with `{"entity":"health"}`.
...
```sh
export KORPO_PRO_APIKEY=sk_live_xxx            # API key
export KORPO_PRO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `korpo-pro_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `korpo-pro_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KORPO_PRO_APIKEY` | API key sent with every request. |
| `KORPO_PRO_BASE` | Optional override of the API base URL. |
...
```sh
./korpo-pro-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"korpo-pro_load","arguments":{"entity":"health","query":{"id":1}}}}'
```
