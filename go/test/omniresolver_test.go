...
import (
	...
	omni "github.com/voxgig-sdk/korpo-pro-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KorpoProError); is {
				out := map[string]any{
					"name":    "KorpoProError",
...
func omniCtx(arg any, client *sdk.KorpoProSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KorpoProSDK); is {
			client = live
