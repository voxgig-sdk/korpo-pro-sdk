...
import (
	...
	"github.com/voxgig-sdk/korpo-pro-sdk/go/core"
...
func TestHealthEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set KORPO_PRO_TEST_HEALTH_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		healthRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.health")))
		...
		_, healthRef01ListOk := healthRef01ListResult.([]any)
...
func healthBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KORPO_PRO_TEST_HEALTH_ENTID")
	...
	idmapResolved := core.ToMapAny(env["KORPO_PRO_TEST_HEALTH_ENTID"])
	...
	if env["KORPO_PRO_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewKorpoProSDK(core.ToMapAny(mergedOpts))
	}
