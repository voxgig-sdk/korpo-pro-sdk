...
class KorpoProTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KorpoProTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KorpoProTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KorpoProTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = KorpoProTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KorpoProTestRunner.getenv(key)
        ...
        explain = KorpoProTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if KorpoProTestRunner._test_control is not None:
            return KorpoProTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KorpoProTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KorpoProTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KorpoProTestRunner.load_test_control()
...
def load_env_local():
    KorpoProTestRunner.load_env_local()


def env_override(m):
    return KorpoProTestRunner.env_override(m)


def entity_data(v):
    return KorpoProTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KorpoProTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KorpoProTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KorpoProTestRunner.load_test_control()


def live_client_options():
    return KorpoProTestRunner.live_client_options()


def live_delay_ms():
    return KorpoProTestRunner.live_delay_ms()
