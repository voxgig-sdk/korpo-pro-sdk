...
class FtCtrl extends KorpoProControl
{
...
class FtClient
{
    ...
    private KorpoProUtility $utility;

    public function __construct(KorpoProUtility $utility, array $options)
    {
    ...
    public function get_utility(): KorpoProUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KorpoProContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KorpoProContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KorpoProContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KorpoProConfig::make_feature($name);
    ...
    private static function buildUrl(KorpoProSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KorpoProError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KorpoProResponse($fetched);
        }
    ...
    private function fail(KorpoProContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KorpoProContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
