...
{
    private static function utility(): KorpoProUtility
    {
        return new KorpoProUtility();
    }
    ...
    private static function ctx(array $over = []): KorpoProContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new KorpoProContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new KorpoProOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof KorpoProError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new KorpoProOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = KorpoProMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new KorpoProOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = KorpoProMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new KorpoProOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = KorpoProMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KorpoProSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = KorpoProMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new KorpoProResponse([]),
            'result' => new KorpoProResult([]),
        ]);
        [, $err] = KorpoProMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new KorpoProSpec([]),
            'response' => null,
            'result' => new KorpoProResult([]),
        ]);
        [, $err] = KorpoProMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new KorpoProSpec(['step' => 's']),
            'response' => new KorpoProResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new KorpoProResult([]),
        ]);
        [, $err] = KorpoProMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new KorpoProSpec(['step' => 's']),
            'response' => new KorpoProResponse(self::resp(200, ['v' => 1])),
            'result' => new KorpoProResult([]),
        ]);
        [, $err] = KorpoProMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new KorpoProSpec(['step' => 's']),
            'response' => new KorpoProResponse(self::resp(200, ['v' => 2])),
            'result' => new KorpoProResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        KorpoProMakeResponse::call($ctx);
    ...
    {
        $preset = new KorpoProResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new KorpoProSpec([]),
            'response' => new KorpoProResponse([]),
            'result' => new KorpoProResult([]),
        ...
        [$got, $err] = KorpoProMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new KorpoProResult([])]);
        [, $err] = KorpoProMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new KorpoProSpec([]), 'result' => null]);
        [, $err] = KorpoProMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KorpoProSpec(['step' => 's']),
            'result' => new KorpoProResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new KorpoProOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KorpoProMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KorpoProSpec(['step' => 's']),
            'result' => new KorpoProResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new KorpoProOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KorpoProMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KorpoProResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new KorpoProSpec([]),
            'result' => new KorpoProResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = KorpoProMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KorpoProMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new KorpoProError('boom', 'boom');
        $utility->fetcher = function (KorpoProContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KorpoProSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = KorpoProMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (KorpoProContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = KorpoProMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(KorpoProResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (KorpoProContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KorpoProSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = KorpoProMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new KorpoProResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new KorpoProSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = KorpoProMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = KorpoProMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = KorpoProMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new KorpoProResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, KorpoProDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new KorpoProResult(['ok' => false])]);
        $this->expectException(KorpoProError::class);
        KorpoProDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new KorpoProResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', KorpoProMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        KorpoProMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new KorpoProBaseFeature();
        $b = new KorpoProBaseFeature();
        KorpoProFeatureAdd::call($ctx, $a);
        KorpoProFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): KorpoProBaseFeature
    {
        $f = new KorpoProBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        KorpoProFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): KorpoProContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new KorpoProSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KorpoProPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new KorpoProResponse(['headers' => null]),
            'result' => new KorpoProResult([]),
        ]);
        KorpoProResultHeaders::call($ctx);
