...
// KorpoPro SDK primary utility test
//
...
//   KorpoProOmni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with KorpoProOmni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   KorpoProOmni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = KorpoProOmni::makeRunner(
                self::TEST_JSON_FILE, KorpoProSDK::test(null, null));
    ...
    private static function client(): KorpoProSDK
    {
    ...
    {
        $client = KorpoProSDK::test(null, null);
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = KorpoProOmni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = KorpoProOmni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = KorpoProOmni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new KorpoProBaseFeature();
    ...
    {
        $hook_client = KorpoProSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends KorpoProBaseFeature {
            public $hook_fn = null;
            public function TestHook(KorpoProContext $ctx): void
            {
    ...
    {
        $init_client = KorpoProSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends KorpoProBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new KorpoProSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_request_basic(): void
    {
        ...
        $this->runsection('makeRequest', function (array $ctxmap) use ($client, $utility) {
            ...
            KorpoProOmni::sync_ctx($ctxmap, $ctx);
            if ($err !== null) {
                throw $err;
            }
            return null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new KorpoProSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = KorpoProSDK::test(null, is_array($setup) ? $setup : null);
        $spec_utility = $spec_client->get_utility();
        $utility = self::client()->get_utility();

        $this->runsection('makeSpec', function (array $ctxmap) use ($spec_client, $spec_utility, $utility) {
            $ctx = KorpoProOmni::ctx_from_map($ctxmap, $spec_client, $spec_utility);
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return KorpoProOmni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = KorpoProSDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = KorpoProOmni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof KorpoProError)
