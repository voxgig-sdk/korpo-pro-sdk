...
class KorpoProMakeError
{
    public static function call(?KorpoProContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new KorpoProContext([], null);
        }
        $op = $ctx->op ?? new KorpoProOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new KorpoProResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof KorpoProError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new KorpoProError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof KorpoProError) {
