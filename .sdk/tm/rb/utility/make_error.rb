...
module KorpoProUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = KorpoProContext.new({}, nil)
    end
    op = ctx.op || KorpoProOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || KorpoProResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(KorpoProError) ? err.msg : err.to_s
    ...
    sdk_err = KorpoProError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(KorpoProError)
