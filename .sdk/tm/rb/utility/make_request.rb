...
module KorpoProUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = KorpoProResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KorpoProResponse.new(fetched)
    else
